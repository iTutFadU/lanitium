...
public class RedirectModifierValue extends SimpleFunctionValue {
    public final CarpetContext context;
    public final RedirectModifier<CommandSourceStack> value;

    protected RedirectModifierValue(CarpetContext context, RedirectModifier<CommandSourceStack> value) {
        super((c, t) -> {
            try {
                return ListValue.wrap(value.apply(CommandContextValue.from((CarpetContext)c, c.getVariable("c").evalValue(c, t))).stream().map(v -> Util.source((CarpetContext)c, v)));
